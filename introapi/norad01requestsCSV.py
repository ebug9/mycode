...
def main():
    # HTTP GET the dataset & split across "\r\n" boundaries
    satellites = requests.get("http://celestrak.org/NORAD/elements/gp.php?GROUP=active&FORMAT=csv").text.split("\r\n")
    #print(satellites)
    ...
    for row in satellites:
        satellitenames.append(row.split(",")[0])
    ...
    # CHALLENGE01 - Write out the list of satelite names to a file
    #with open('satelite_names.txt', 'w') as satnamesfile:
    #    for sat in satellitenames:
    #        satnamesfile.write(sat+"\n")
